...
function GuestMomentPreview({
  ...
  // AIT-134/159: mobile (≤640px) compact chrome so Lab sample + primary Sign-in
  // stay above the fold on ~390px. After AIT-152 full tab hide on /create, bottom
  // pad is safe-area only (no tab ghost / double pad). Desktop/lg keeps archive
  // density.
  return (
    <section
      className="relative isolate min-h-[calc(100vh-56px)] overflow-hidden bg-[var(--void)] px-4 pb-[max(1.25rem,env(safe-area-inset-bottom,0px))] pt-3 text-[var(--cream)] sm:px-7 sm:pb-10 sm:pt-6 lg:px-10 lg:py-7"
    ...
    >
      ...
      <div className="mx-auto grid max-w-[1360px] gap-3 sm:gap-7 lg:min-h-[calc(100vh-7rem)] lg:grid-cols-[minmax(220px,0.75fr)_minmax(330px,390px)_minmax(260px,0.78fr)] lg:items-center lg:gap-10 xl:gap-14">
        {/* Mobile order: sample → primary path → title chrome (fold priority). */}
        ...
        <section
          className="order-1 mx-auto w-full max-w-[200px] sm:max-w-[320px] lg:order-2 lg:max-w-[390px]"
        ...
        >
          <div className="relative rounded-[22px] border border-white/12 bg-[var(--card)] p-1.5 shadow-[0_42px_120px_-42px_rgba(255,32,122,0.62)] sm:rounded-[30px] sm:p-2">
            {/* AIT-159: after tab hide, allow a bit more sample height (~46svh) while
                keeping Sign-in on the first screen at ~390×844. */}
            <div className="relative aspect-[9/16] max-h-[min(46svh,21rem)] overflow-hidden rounded-[17px] bg-black sm:max-h-[min(62vh,32rem)] sm:rounded-[23px] lg:max-h-[calc(100vh-11rem)]">
              <AutoPlayVideo
            ...
            {sessionBoot === "timeout" ? (
              ...
              >
                ...
                <button
                  ...
                  className="mt-2 inline-flex min-h-10 items-center justify-center rounded-full bg-white px-4 text-[10px] font-black uppercase tracking-[0.12em] text-black transition hover:bg-white/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00D9FF]"
                >
                  Retry access check
                </button>
              </div>
