...
export function waitPhaseForElapsed(
  ...
  if (elapsed < 140) {
    return {
      ...
      detail: "Longer clips take longer — do not refresh",
      step: 2,
    };
  }
  return {
    id: "long",
    title: "Almost there",
    detail: "Finishing frames · cancel only if you must leave",
    step: 2,
  };
}
      ...
      {onCancel ? (
        <>
          <button
            type="button"
            onClick={onCancel}
            className="relative mt-4 rounded-full border border-white/20 bg-black/40 px-4 py-1.5 text-[11px] font-bold text-white/80 backdrop-blur transition hover:border-white/40 hover:text-white"
          ...
          <p className="relative mt-1.5 max-w-[16rem] text-[10px] leading-relaxed text-white/35">
