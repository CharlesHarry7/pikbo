...
export function VideoTile({
  ...
  return (
    <Link
      href={item.href}
      className="video-tile group block overflow-hidden rounded-xl border border-white/[0.08] bg-black shadow-[0_12px_32px_-14px_rgba(0,0,0,0.85)] transition duration-300 hover:-translate-y-0.5 hover:border-neon-pink/45 hover:shadow-[0_20px_48px_-16px_color-mix(in_srgb,var(--neon-pink)_18%,transparent)]"
    ...
    >
      <div className={`relative ${aspectClass(item.ratio, compact)}`}>
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-neon-pink/45 to-transparent opacity-0 transition group-hover:opacity-100" />
        ...
        {!isConcept ? (
          <span className="absolute right-2 top-2 rounded-full bg-neon-pink px-1.5 py-0.5 text-[8px] font-black text-void opacity-0 shadow transition group-hover:opacity-100">
            Remake
          </span>
        ) : null}
        <div
        ...
        >
          ...
          {compact ? (
            <p className="mt-1 text-[9px] font-bold uppercase tracking-wide text-neon-pink opacity-0 transition group-hover:opacity-100">
              {isConcept ? "View recipe notes →" : "Your photo →"}
            </p>
          ) : (
            <p className="mt-1.5 inline-flex items-center gap-1 rounded-full bg-neon-pink/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-neon-pink ring-1 ring-neon-pink/30 transition group-hover:bg-neon-pink group-hover:text-void">
