...
export function FlowMediaCard({
  ...
  return (
    <Link
      href={href}
      className="group relative overflow-hidden rounded-2xl bg-neutral-900 ring-1 ring-white/10 transition duration-300 hover:-translate-y-1 hover:shadow-[0_20px_50px_-28px_rgba(196,165,116,0.2)] hover:ring-[var(--brand)]/50"
    ...
    >
      <div className="relative aspect-video overflow-hidden">
          ...
          <span
            className={`rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide shadow-sm ${
              isPreview
                ...
                : "bg-black/65 text-[var(--brand)] backdrop-blur-sm"
            }`}
          >
            {badge}
          </span>
        ...
        <span className="absolute bottom-2 left-2 translate-y-1 rounded-full bg-[var(--brand)] px-2.5 py-1 text-[10px] font-black text-[var(--primary-foreground)] opacity-0 shadow-[0_0_20px_rgba(196,165,116,0.35)] transition duration-300 group-hover:translate-y-0 group-hover:opacity-100">
          {ctaLabel}
        </span>
      </div>
      <div className="p-4">
        <h3 className="text-lg font-bold tracking-tight text-white transition group-hover:text-[var(--brand)]">
          {title}
        </h3>
        <p className="mt-1 text-sm leading-snug text-white/50">{blurb}</p>
        <p className="mt-3 text-[11px] font-bold uppercase tracking-wide text-[var(--brand)]/90">
