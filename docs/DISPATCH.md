...
|---|---|---|---|
| Codex | 队列收口、R2 结案与 R0 最终审查 | PR #45 · `agent/gpt/active-queue-reset` | 记录 #44 已合并；关闭重复任务；只在 #42 修复 capture 异常成本漏洞后放行 |
| Grok | 预留积分异常退出安全网 | PR #42 | capture RPC 抛异常时必须 withhold，不能 release；修复 STATUS 表格并让真实 CI 执行行为测试 |
| WorkBuddy | 外链历史证据合并 | PR #41 | 只改证据/日志；合并历史结论；不碰产品代码、数据库、部署或新提交；#46 已关闭 |
...
- #42 和 #41 结束后，下一条产品主线只有：让真实卖家在私有预览完成已合并的“一张图 → 三条素材 → 导出 Launch Pack”，形成可审计证明；不是继续扩张 SEO。
...
2. 必须补 capture 后异常的行为测试：settle 抛错后 release backend 调用数为 0，输出进入 withhold；不用源码正则代替账务行为测试。
3. 完成后等待 Codex 下一条明确派工。

### WorkBuddy —— `agent/workbuddy/...`
1. 当前只处理 PR #41，把重复外链报告收敛成一份事实记录。
2. #46 已因重复占用和跳级关闭；不得在 #41 完成前新开分支或 SEO/外链任务。
