...
| # | 老板最小动作 | 用途 |
|---|--------------|------|
| A | 核对 Supabase `URL` + `anon` + `service_role` 已写入 Vercel Production/Preview（本机同名配置存在，但不代表 Vercel 已配置） | 打开 magic link + durable RPC |
...
| E | GitHub Actions 实际工作流已由 Codex 同步；确认 PR/main 的 Actions 运行全绿 | live CI fail-closed（R3） |
...
| Supabase magic-link + claim + guest migrate | Code on main; local env names present; **Vercel env + SQL/Auth console still unverified** — missing runtime config returns `NOT_CONFIGURED` |
