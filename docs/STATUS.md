...
| GROK-GROWTH | Global-English SERP, seller intent, content-gap and attainable-link evidence | Grok | **review** | agent/grok/pikbo-growth-evidence | Delivered `docs/growth/SERP_INTENT_EVIDENCE_2026-07-28.md` + `LINK_OPPORTUNITIES_2026-07-28.csv`; every conclusion/row sourced + Beijing timestamp; no volume invention, page gen, directory submit or main push |
| WB-SEO-BASELINE | AITDK/GSC/browser baseline with URL, timestamp and before/after evidence | WorkBuddy | **review** | agent/workbuddy/seo-baseline-2026-07-28 | Done read-only 2026-07-28: 16 impressions / 0 clicks / avg pos 4.1 (tiny sample); GSC 30 indexed / 67 not (data 7/24) — will drop by design: c914eac noindex slimming confirmed live on 8/10 sampled "indexed" URLs; sitemap now 7 URLs all indexable; report + timestamped artifacts in docs/evidence/WORKBUDDY_SEO_BASELINE_2026-07-28.md; no indexing request, code change, secrets, database, deploy or direct-main push |
| SELLER-GOLD | Seller Pack cached golden path + failure/retry/partial-success regression | Codex / Grok | **review** | main | Grok: `seller-pack-cached-smoke` on main (contract + pure policy + CI). Codex may still add browser e2e golden path on branch |
