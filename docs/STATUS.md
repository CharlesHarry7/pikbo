...
Library statuses + failure same-photo handoff (#141), reconciliation hardening,
honest guest Create proof, and the exact private Preview auth origin.
In-flight: PR #142 (Create receiver for private photo reuse), PR #139
(one-SKU acceptance harness), and success-row same-photo reuse CTA (this PR).
...
|---|---|---|---|---|
| P0 | Owned photo produces the actual private Moment | #54 | protected Preview health green; code path nearly complete on main + #142; blocked on FAL `$0.00`, migration apply, merge of #142 | owner Magic Link → owned photo → one `9:16` / 5s / 720p Street Power-Up → private object → Library/download |
...
- #141: durable Library statuses across open/terminal rows; terminal failures
  ...
  idempotency keys).
- AIT-18 (this PR): deliverable Library successes with a bound photo also emit
  the controlled same-photo Create handoff and a **Same photo · new Moment**
  CTA (download remains primary). Create receiver still depends on #142.
...
2. Merge this success-row reuse CTA (or land after #142) so Library download
   and same-photo reuse both work after a real result.
3. Apply #140 migration on the protected non-prod database.
4. Fund the dedicated FAL account, then use the owner Magic Link (or the
