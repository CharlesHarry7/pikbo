...
| R0 | Disable anonymous live generation; cached demo only until authenticated durable reserve | Claude / Grok / Codex | **review** | main via PR #40 | Route fail-closed plus `t5-r0-critical-path` source locks and pure gate pass; anonymous never reaches the provider. Live multi-node still needs verified Vercel env and applied T5/R1 SQL. |
