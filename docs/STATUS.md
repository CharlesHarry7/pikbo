...
| R1b | Exact retry token + fixed deadline + worker/read separation | Claude | review | agent/claude/retry-deadline | Retry requires exact parent→child ID and one-time token; polls never extend deadline; canceled/late provider output is withheld |
| R1c | Durable settlement reconciliation worker | Claude | review | agent/claude/durable-reconciliation | Source-only event queue, worker lease and atomic finish RPC; late/capture-failed output is persisted withheld. Non-prod SQL rehearsal still required |
