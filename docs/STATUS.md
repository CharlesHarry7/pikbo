...
| H-RETENTION | Proof-first toy seller homepage | Grok | **review** | agent/grok/home-proof-first | One Seedance Lab premiere + compact upload in viewport; source/output proof; 8 distinct cached projects; one-video playback budget |
| EFFECT-TRUTH | Exact demo vs concept separation in Effects | Grok | **review** | agent/grok/effects-proof-truth | No borrowed loops; static No demo yet cards; exact media registry and JSON-LD gate |
...
| T6 | Server-side free watermark (ffmpeg) | Grok | **blocked** | agent/grok/t6-watermark-worker-v1 | Source-only server-owned derivative worker skeleton; Free raw URLs remain hidden/blocked until persistent worker + ffmpeg/ffprobe proof |
...
| UI-q | UI quality r1–r3 aesthetic | Grok | review | agent/gpt/product-core-integration | Create + Seller Pack first-run 390px: plain Create has no fabricated Remix, recipe before collapsed Lab samples, Identity Advanced; pack steps/sticky upload-generate-library-retry smoke locked |
...
| T6 | Server-side free watermark (ffmpeg) | Grok | **blocked** | agent/grok/t6-watermark-worker-v1 | Source-only worker skeleton; file-level proof requires persistent worker plus ffmpeg/ffprobe |
