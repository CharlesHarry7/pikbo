...
|---|---|---|---|---|---|
| PRO-RESULT-SELECTOR | Replace the rejected Mothcat/editorial front door with a Pro-directed toy-style + three-format result selector while keeping public/private truth gates unchanged | Codex / Grok / WorkBuddy | doing | `agent/gpt/pro-result-selector` | Clean `origin/main@2cfb584` baseline. Grok and context audits forbid backend changes and false verified-media claims; WorkBuddy requires CTA + one visual result in the 390px first screen. Full regression, Preview and frozen-diff review pending. |
