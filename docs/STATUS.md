...
| SELLER-GOLD | Seller Pack cached golden path + failure/retry/partial-success regression | Codex / Grok | **review** | agent/gpt/seller-pack-cached-golden-path | Real contract/recovery/retry modules exercised, running-server cached API trio added, zero provider/credit assertions and partial-success retention locked; final build + PR/CI pending |
...
| R0 | Disable anonymous live generation; cached demo only until authenticated durable reserve | Claude / Grok | **review** | main + agent/grok/t5-auth-credits-smoke | Route fail-closed on main; Grok added `t5-r0-critical-path` source locks + pure gate (anonymous never provider). Live multi-node still needs T5 SQL/keys. |
...
| GROK-TAKEOVER | Complete every remaining no-cost product, engineering, QA and private-release task | Grok | doing | agent/grok/t5-auth-credits-smoke | T5/R0 code-side smokes on PR #40. **Boss blockers:** BLOCKERS T5 A–E (keys/SQL/workflow). T6/Vercel DNS still blocked. |
...
| T5 | Supabase auth + durable credits | Grok | **review** | agent/grok/t5-auth-credits-smoke | Code-side complete on branch: pure engine smoke + **t5-r0-critical-path** (R0 gate, refund/idempotency source locks, magic-link NOT_CONFIGURED). **BLOCKED multi-node:** Supabase keys + SQL apply (BLOCKERS T5 A–E). Evidence: T5_AUTH_CREDITS_SMOKE + T5_R0_CRITICAL_PATH 2026-07-28. PR #40. |
