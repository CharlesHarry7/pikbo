...
| R0-NET | Unexpected-exit reservation safety without double release/refund | Grok | **review** | PR #42 · `agent/grok/t5-r0-refund-idempotent` | Capture throw/fail auto-withhold (0 release). Behavioral `r0-safety-net` in `docs/ci` (agent cannot push live workflow without `workflow` scope — Codex re-sync after review). Rebased on main `e0ab4f0`. |
...
| R0 | Disable anonymous live generation; cached demo only until authenticated durable reserve | Claude / Grok / Codex | **review** | main via PR #43; residual **PR #42** | Anonymous gate on main. PR #42: lifecycle release≤1; capture throw/fail → withhold (0 release backend). Awaiting Codex review. |
