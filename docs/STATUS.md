...
|---|---|---|---|---|---|
| PRIVATE-PREVIEW-INPUT | Persist one invited seller's owned input, bind it atomically to all three Pack jobs, and recover the Pack from the account in Create/Library | Codex / Grok / WorkBuddy | review | `codex/private-preview-readiness-validation` | GPT Pro, Grok and WorkBuddy GO for commit/PR plus protected Preview only. Private `toy_assets` upload/verification, immutable three-child binding, owner discovery, exact Library recovery, authenticated named downloads, stale-pointer fallback and safe login return are implemented; legacy unbound Packs fail closed. Full source gate and 199-route build pass. Migration/bucket/RPC remain unapplied, so Preview/production readiness stays false and no Provider, Stripe, pricing, credit-rule or public-Live behavior changed. |
