...
| PRESET-FRONTDOOR | Apply only the safe outcome/upload patterns from the purchased four-repository ShipAny bundle to the existing Seller Pack | Codex / Grok / WorkBuddy | review | `agent/gpt/preset-first-frontdoor` · PR #93 | The four purchased repositories were audited as references, not migration targets. Pikbo keeps its existing private Storage, fixed three-job Pack, durable atomic credits, recovery and Stripe rehearsal; the generic public upload, provider-first task creation, second wallet/Auth, Admin, model marketplace and framework variants are rejected. The accepted slice is limited to truthful private-input progress plus a fail-closed signed PUT: a non-2xx upload stops before verification, 30-credit reserve or Provider work. Grok APPROVE `49ba14ad-b47c-479d-af40-4f76dac183de`; WorkBuddy task `审计 Pikbo 创建页面` APPROVE for desktop/390px public Create, while Codex Chrome separately proves the exact Seller-Pack URL has 0 file inputs, 0 purchase CTAs and 0 overflow. Full fixed suite and 198-route build PASS. No production/public/paid gate changed. |
