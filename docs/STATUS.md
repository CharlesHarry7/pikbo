...
|---|---|---|---|---|---|
| PRIVATE-PREVIEW-INPUT | Persist one invited seller's owned input, bind it atomically to all three Pack jobs, and recover the Pack from the account in Create/Library | Codex / Grok / WorkBuddy | review | PR #97 merged at `4659460`; evidence branch `codex/nonprod-private-input-rls-evidence` | DB/RLS plus a locked zero-Provider harness now pass on `pikbo-private-preview-56`: one synthetic input, one idempotent 30-credit Pack, exactly three same-input jobs, owner-scoped v2 status-RPC recovery, A-visible/B-hidden RLS, direct authenticated input-bucket reads denied, Provider/Stripe 0 and final residue 0. The run also proved canonical/runtime drift: the DB requires stronger v2 asset/status contracts while `main` still writes simplified v1 data. Protected Preview remains `privatePreview=false`, Provider USD 0 and paid false. HTTP upload/recovery, Provider, production and paid launch remain NO-GO until the minimal v2 adapter alignment selected by GPT Pro is complete. See `docs/evidence/PRIVATE_INPUT_NONPROD_RLS_GATE_2026-08-02.md`. |
