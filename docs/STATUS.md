...
|---|---|---|---|---|---|
| PRIVATE-PREVIEW-INPUT | Persist one invited seller's owned input, bind it atomically to all three Pack jobs, and recover the Pack from the account in Create/Library | Codex / Grok / WorkBuddy | review | `agent/gpt/private-input-admission-readiness`; PR pending; based on main `fa362b8` | Private input admission is separated from full Provider readiness: protected Preview may prove signed upload and owner completion while reserve/generate/credits/Stripe remain closed. GPT Pro chose the boundary; Grok APPROVE P0=0; WorkBuddy blocked and then approved the corrected seller-truth delta. Full local gate and 199-page build pass. Production remains `privatePreview=false`, Provider USD 0 and paid false. |
