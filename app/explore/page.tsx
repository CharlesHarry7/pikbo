...
const EXPLORE_FAQ = [
  ...
  },
  {
    ...
    a: "No. Cached Lab playback costs 0 credits and never processes your upload. Live remains gated; when Live is enabled for an eligible account, Generate shows the exact credit quote before submission.",
  },
] as const;
    ...
    >
      ...
      <header className="sticky top-0 z-20 border-b border-white/10 bg-black/90 px-4 py-4 backdrop-blur sm:px-6">
        <div className="mx-auto flex max-w-7xl flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[var(--brand)]">
          ...
          >
            ...
            <Link
              ...
              className="rounded-full border border-[var(--brand)]/40 bg-[var(--brand)]/10 px-5 py-2.5 text-xs font-black text-[var(--brand)] transition hover:bg-[var(--brand)]/15"
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex flex-col items-start justify-between gap-4 rounded-2xl border border-white/10 bg-white/[0.04] p-5 sm:flex-row sm:items-center">
          ...
          <Link
            ...
            className="shrink-0 rounded-full border border-[var(--brand)]/40 px-5 py-2.5 text-xs font-black text-[var(--brand)]"
          >
            Create one Moment →
          </Link>
        </div>
