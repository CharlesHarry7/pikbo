...
export default function ImageStudioPage() {
   ...
   * Same pure gate as Create / Landing / Batch (`canRetryGenerateFailure`).
   */
  ...
  useEffect(() => {
    const t = window.setTimeout(() => {
      ...
      // One-time bearer is stored under pikbo_retry_token:{retryJobId} (not in URL).
      try {
        ...
        if (retry && retry.length >= 8) {
          const jobId = retry.slice(0, 128);
          try {
            const token = sessionStorage.getItem(`pikbo_retry_token:${jobId}`);
            if (token && token.length >= 16) {
              retryHandoffRef.current = {
                retryJobId: jobId,
                retryToken: token,
              };
              sessionStorage.removeItem(`pikbo_retry_token:${jobId}`);
            }
          } catch {
  ...
  async function cancelSessionStill(jobId: string) {
    try {
      // Bearer so owner durable UUID can return DURABLE_NO_CANCEL (AIT-485).
      ...
      if (!res.ok) {
        // Never invent silent “canceled” for durable UUIDs.
        if (
          data.code === "DURABLE_NO_CANCEL" ||
          data.code === "DURABLE_DETAIL_UNAVAILABLE"
        ) {
          setError(
            typeof data.message === "string"
              ? data.message
              : data.code === "DURABLE_DETAIL_UNAVAILABLE"
                ? "Private Library could not verify this still for Cancel — try again when storage is ready"
  ...
  async function retrySessionStill(j: SessionStillJob) {
    ...
    // Closed gates (auth / paywall / durable hold) never invent process-memory Retry.
    if (
      !canRetryGenerateFailure({
        code: j.errorCode,
        busy,
        hasInput: nextPrompt.trim().length >= 4,
      })
    ) {
      setError(
        j.error ||
          "This still cannot be retried here — change access, balance, or wait for settlement, then start a new attempt"
      );
      recordFailGate({ code: j.errorCode || null });
      return;
    }
  ...
  async function generate(opts?: { prompt?: string; aspect?: string }) {
    ...
    try {
      ...
      const result = await postImageWithRetry(
        {
          ...
          ...(ledgerRetry
            ? {
                retryJobId: ledgerRetry.retryJobId,
                retryToken: ledgerRetry.retryToken,
              }
            : {}),
        },
      ...
      if (!result.ok) {
        ...
        // Only creditsRefunded === true may claim restored (never invent).
  ...
  const canRetryStillFail = canRetryGenerateFailure({
    code: lastFailCode,
    fatal: lastFailFatal,
