  ...
  return (
    <div className="min-h-screen bg-black pb-28 text-white">
        ...
        >
          {sections.map((s) => (
            <a
              key={s.id}
              href={`#flow-${s.id}`}
              className="shrink-0 rounded-full border border-white/12 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-white/50 hover:border-[var(--neon-pink)]/40 hover:text-[var(--neon-pink)]"
            >
      ...
      <div className="mx-auto max-w-[1400px] px-4 py-10 text-center">
        ...
        <Link
          href={FLOW_GENERATE_HREF}
          className="mt-6 inline-flex rounded-full bg-[var(--neon-pink)] px-8 py-3.5 text-sm font-black text-[var(--void)] shadow-[0_0_32px_rgba(196,165,116,0.25)]"
