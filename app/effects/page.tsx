...
const EFFECTS_FAQ = [
  ...
  },
  {
    ...
    a: "Watching Lab demos costs 0 and never processes your upload. Live remains gated; when Live is enabled for an eligible account, Generate shows the exact credit quote before submission. Lab demos stay free either way.",
  },
  {
        ...
        <div className="mt-3 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none]">
          {jobBlocks.map((w) => (
            <Link
              key={w.id}
              href={w.href}
              className="shrink-0 rounded-full border border-[var(--mint)]/35 bg-[var(--mint)]/[0.1] px-3 py-1.5 text-[11px] font-semibold text-[var(--mint)] shadow-[0_0_20px_rgba(196,165,116,0.08)] transition hover:border-[var(--mint)] hover:bg-[var(--mint)]/15"
            >
              {w.emoji} {w.label}
            </Link>
          ))}
