        ...
        <div className="mt-3 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none]">
          {jobBlocks.map((w) => (
            <Link
              key={w.id}
              href={w.href}
              className="shrink-0 rounded-full border border-[var(--neon-pink)]/35 bg-[var(--neon-pink)]/[0.1] px-3 py-1.5 text-[11px] font-semibold text-[var(--neon-pink)] shadow-[0_0_20px_rgba(255,78,205,0.08)] transition hover:border-[var(--neon-pink)] hover:bg-[var(--neon-pink)]/15"
            >
              {w.emoji} {w.label}
            </Link>
          ))}
          {groups.map(({ category }) => (
            <a
              ...
              className="shrink-0 rounded-full border border-white/12 bg-white/[0.03] px-3 py-1.5 text-[11px] font-semibold text-white/55 transition hover:border-[var(--neon-pink)]/40 hover:text-[var(--neon-pink)]"
            >
              {category.label}
            </a>
          ))}
