 ...
 * - fail Retry is server-gated (no fake retriable path / no process-memory Retry
 *   invent while durable hold is open)
 * - refund confirmed only when creditsRefunded === true (never invent restore)
 */
 ...
 *
 * DURABLE_CREDITS_UNAVAILABLE = hold/reconcile open — never invent process-memory
 * Retry while durable settlement is unresolved (fail-closed residual).
 */
 ...
 * - durable credit hold never invents a process-memory Retry path
 */
