...
export async function fetchMe(opts?: {
  /**
   * Wall-clock bound for Studio / chrome open so we never stay on "Opening…"
   * or permanent "…" forever. Covers BOTH supabase getSession (authHeaders)
   * and /api/me — a hanging session lookup used to leave chrome stuck even
   * when fetch had AbortController.
