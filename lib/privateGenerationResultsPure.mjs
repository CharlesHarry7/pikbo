...
/**
 * Button label for durable deliverable success same-photo reuse CTA.
 * Present only when the client accepted a controlled Create handoff URL.
 * This is a new Moment with the same verified photo — never a replay of the
 * old job or its idempotency key.
 */
export function librarySuccessSamePhotoReuseLabel() {
  return "Same photo · new Moment";
}
...
export function privateLibraryJobFromRow(row) {
  ...
  const job = {
    ...
    capabilities: {
      ...
      localRetry: false,
      localCancel: false,
      // Terminal failure: honest Create new-attempt CTA (same-photo when URL set).
      newAttempt: terminalFailure,
      // Deliverable success: reuse verified photo for another Moment (not a
      // replay). Client still requires accepted newAttemptUrl before showing.
      reuseSamePhoto: deliverable,
      refreshOnly: open,
    },
  };
  ...
  // - terminal failure → recovery new-attempt (newAttempt CTA)
  // - deliverable success → same-photo reuse for another Moment (reuse CTA)
  // Missing/invalid keeps generic Create fallback. Never reuses the old job's
  // idempotency key — always a fresh attempt.
  if (terminalFailure || deliverable) {
