...
const SAFE_LIBRARY_ERROR_CODES = new Set([
  ...
  /** Succeeded ledger row without a private video object — not downloadable. */
  "PRIVATE_RESULT_MISSING",
  "LIVE_DISABLED",
...
export function libraryDurableTerminalFailureCopy(input) {
  ...
  if (
    input.errorCode === "PRIVATE_RESULT_MISSING" ||
    (input.status === "succeeded" && input.errorCode === "PRIVATE_RESULT_MISSING")
  ) {
    return libraryMissingPrivateResultCopy({ samePhotoHandoff: samePhoto });
  }
...
/**
 * Succeeded ledger row without a private deliverable object.
 * Never invents a download, never claims refund completed.
 *
 * @param {{ samePhotoHandoff?: boolean }} input
 */
export function libraryMissingPrivateResultCopy(input = {}) {
  const samePhoto = input.samePhotoHandoff === true;
  return samePhoto
    ? "This Moment settled without a private downloadable file. Credit settlement may be unconfirmed — start a new attempt with the same verified photo."
    : "This Moment settled without a private downloadable file. Credit settlement may be unconfirmed — start a new Moment; same-photo retry is not available yet.";
}

/**
 * True when a Library DTO is a success row that cannot be downloaded
 * (missing private object, blocked Free live, or settlement unconfirmed).
 *
 * @param {{ status?: string, downloadAllowed?: boolean }} job
 */
export function libraryResultMissingDeliverable(job) {
  if (!job || typeof job !== "object") return false;
  return job.status === "succeeded" && job.downloadAllowed !== true;
}
...
export function privateLibraryJobFromRow(row) {
  ...
  /** Succeeded without a private video object — not a downloadable money path. */
  const missingPrivateResult = status === "succeeded" && !deliverable;
  ...
  // New attempt for terminal fails AND missing private deliverable.
  const allowNewAttempt = terminalFailure || missingPrivateResult;
  ...
  } else if (missingPrivateResult) {
    // Never invent a download URL or claim capture/refund completed.
    job.creditsOutcome = "refund unconfirmed";
    job.errorCode = "PRIVATE_RESULT_MISSING";
  ...
  // Controlled Create URL when the durable row has a UUID-shaped input asset.
  // Missing/invalid keeps generic Create fallback. Never reuses the old job's
  // idempotency key — this is a new attempt.
  if (allowNewAttempt) {
