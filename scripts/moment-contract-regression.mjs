...
assertMatch(
  studio,
  /const fixedMomentNextPath\s*=\s*fixedMomentCreateReturnPath\(\{[\s\S]{0,220}assetId:\s*initialAssetId/,
  "fixed Moment gate must derive its sign-in return path from fixedMomentCreateReturnPath (preserves assetId handoff)"
);
assertMatch(
  studio,
