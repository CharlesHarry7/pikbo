...
// AIT-489 / AIT-320: four-surface money path gallery-calm copper board tokens
// (Home tokens already copper; Create/Library/Pricing + shared shell chrome align)
{
  const lime = /#c8ff3d|c8ff3d|200\s*,\s*255\s*,\s*61/i;
  // Pre-gallery carnival neon — hard-coded pink/purple/cyan must not return
  const carnival =
    /#ff4ecd|#b14eff|#00d9ff|#00ffa3|255\s*,\s*78\s*,\s*205|177\s*,\s*78\s*,\s*255|0\s*,\s*217\s*,\s*255/i;
  const fourSurface = {
    ...
    "components/HomeCinemaHero.tsx": read("components/HomeCinemaHero.tsx"),
    "components/HomeDesignerGallery.tsx": read(
    ...
    "components/AppShell.tsx": read("components/AppShell.tsx"),
  ...
  for (const [rel, src] of Object.entries(fourSurface)) {
    ...
    assert(
      !carnival.test(src),
      `${rel} must not hard-code carnival neon (pink/purple/cyan); use gallery-calm copper board`
    );
  }
  assert(
    ...
    "Home rails + Library + browse CTA use neon-pink board tokens (copper alias)"
  );
  assert(
    fourSurface["components/HomeCinemaHero.tsx"].includes("var(--brand)") &&
      fourSurface["app/create/page.tsx"].includes("var(--brand)") &&
      fourSurface["app/library/page.tsx"].includes("var(--brand)") &&
      fourSurface["app/pricing/page.tsx"].includes("var(--brand)"),
    "Home/Create/Library/Pricing page chrome use brand board tokens"
  );
  assert(
    fourSurface["components/CreateStudio.tsx"].includes("rgba(196,165,116") &&
    ...
    "Create + Pricing glows use gallery-calm copper rgba board tokens"
  );
}
