...
// AIT-470: /for hub residual lime → neon-pink board tokens
{
  const forPage = read("app/for/page.tsx");
  assert(
    !/#c8ff3d|c8ff3d|200\s*,\s*255\s*,\s*61|var\(--mint\)/.test(forPage),
    "For hub page must not hard-code competitor lime (#c8ff3d / rgba 200,255,61 / mint)"
  );
  assert(
    forPage.includes("var(--neon-pink)") &&
      forPage.includes("rgba(255,78,205"),
    "For hub primary chrome uses neon-pink board tokens"
  );
}
