...
// AIT-478: /guides hub residual lime → neon-pink board tokens
{
  const guidesPage = read("app/guides/page.tsx");
  assert(
    !/#c8ff3d|c8ff3d|200\s*,\s*255\s*,\s*61|var\(--mint\)/.test(guidesPage),
    "Guides hub page must not hard-code competitor lime (#c8ff3d / rgba 200,255,61 / mint)"
  );
  assert(
    guidesPage.includes("var(--neon-pink)") &&
      guidesPage.includes("rgba(255,78,205"),
    "Guides hub primary chrome uses neon-pink board tokens"
  );
}
